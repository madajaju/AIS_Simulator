...
const TIME_WINDOW = 7 * 60 * 1000; // 7 minutes
const HOUR_WINDOW = 24; // 24 hours
...
const _processUSV = (usvs) => {
    ...
    for (let fi in usvs) {
        ...
        for (let ti in timeKeys) {
            ...
            for(let loc in uShips) {
                let upoints = uShips[loc];
                if(upoints.length > 1) {
                    for (let offset = -1; offset <= 1; offset++) {
                        let key = timeKey + offset;
                        if (_ships[dateKey].time.hasOwnProperty(key)) {
                            let locations = _ships[dateKey].time[key];
                            if(locations.hasOwnProperty(loc)) {
                                let ships = locations[loc];
                                if(ships.length !== upoints.length) {
                                    if (!_stats.hasOwnProperty(loc)) {
                                        _stats[loc] = 0;
                                    }
                                    _stats[loc]++;
                                }
...
const _parseUSVLine = (line) => {
    ...
    if (!_usv[timeKey].hasOwnProperty(locKey)) {
        _usv[timeKey][locKey] = [];
    }
    _usv[timeKey][locKey].push({MMSI: MMSI, BaseDateTime: baseTime, LAT: Number(LAT), LONG: Number(LONG), SOG: SOG, COG: COG});
}
